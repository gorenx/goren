	...
	if actualPath != ":memory:" {
		...
		if err := os.MkdirAll(filepath.Dir(actualPath), 0o700); err != nil {
			return nil, "", fmt.Errorf(
				"session projection cache sqlite: create database directory: %w",
				err,
			)
		}
		...
		if createErr != nil && !errors.Is(createErr, os.ErrExist) {
			return nil, "", fmt.Errorf(
				"session projection cache sqlite: create database file: %w",
				createErr,
			)
		}
	}
...
func configureDatabase(
	...
	if storedApplicationID != 0 && storedApplicationID != applicationID {
		return fmt.Errorf(
			"session projection cache sqlite: database %q belongs to another application",
			actualPath,
		)
	}
	if storedApplicationID == 0 && (onDisk != 0 || len(tables) != 0) {
		return fmt.Errorf(
			"session projection cache sqlite: database %q is not an empty checkpoint store",
			actualPath,
		)
	}
	if storedApplicationID == applicationID {
		for _, tableName := range tables {
			if _, recognized := recognizedTables[tableName]; !recognized {
				return fmt.Errorf(
					"session projection cache sqlite: database %q contains unrecognized table %q",
					actualPath,
					tableName,
				)
			}
