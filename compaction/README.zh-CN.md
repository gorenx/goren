...
- Provider 和未来 Commands Consumer 共享的结果与错误分类。

本包不拥有 threshold、retention、summarization prompt、Token Meter replay、Tool result pruning、Agent hook 注册、Session persistence 或 `/compact` 命令。
...
flowchart LR
    ...
    COMMAND[Future command Consumer] --> ENGINE
...
- Event/config/result/checkpoint 与 `b150a55` TypeScript vectors 已有可重复生成的差分契约。
