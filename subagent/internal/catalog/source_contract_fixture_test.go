...
func catalogSourceOutput(t *testing.T, scriptName string) []byte {
	t.Helper()
...
func catalogContractJSON(t *testing.T, entries any) any {
	t.Helper()
...
func sourceCatalogJSON(t *testing.T, rawValue []byte) any {
	t.Helper()
	var decoded any
	if decodeErr := json.Unmarshal(rawValue, &decoded); decodeErr != nil {
		t.Fatal(decodeErr)
...
func catalogContractEntries(entries []subagent.ListEntry) []map[string]any {
	result := make([]map[string]any, 0, len(entries))
	for _, entry := range entries {
		result = append(result, catalogContractEntry(entry))
	}
	return result
}

func catalogContractDescendants(
	entries []subagent.DescendantListEntry,
) []map[string]any {
	result := make([]map[string]any, 0, len(entries))
	for _, descendantEntry := range entries {
		entry := catalogContractEntry(descendantEntry.Entry)
...
func catalogContractEntry(entry subagent.ListEntry) map[string]any {
...
func newContractCatalogSession(
	t *testing.T,
