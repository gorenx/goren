...
- retry policy 的延时、jitter 和次数决策；这些由[`llmretry`](../llm/retry/README.zh-CN.md)负责，Agent Loop 只提供 failed attempt 的 retry seam 并执行明确返回的 retry action；
...
- 默认 RetryPolicy 的 delay/jitter/attempt Consumer 由[`llmretry`](../llm/retry/README.zh-CN.md)作为独立 Plugin 监听 `agent/request-error`，不能塞进 DeepSeek Adapter 或硬编码在 Agent Loop；
