...
| S5-D10 | 优化 | Session Projection Cache、独立 SQLite checkpoint store 与 cold read 接入 | Completed | Race Verified：`CheckpointCache`、Factory/Plugin/Assembly、API list/history 与 Subagent ChildDirectory 已闭环；cache 可丢弃且不替代 Session facts，专项证据见[进度矩阵](./SessionProjectionCache实施进度矩阵.md) |
...
| S6-Q01 | Session Query | consumer-owned `QueryService` 与 typed failure taxonomy | Completed | Go Verified：服务接口覆盖 exact reads、filter、title、surface、event、trace 与 full-text；API/SQLite 类型不进入 contract |
| S6-Q02 | Session Query | live-preferred logical corpus 与 source conflict | Completed | Go Verified：live 覆盖同 ID persisted log但保留双 availability；Header conflict、cold-to-live transition 与 replay validation 明确处理 |
| S6-Q03 | Session Query | exact list/read/filter/title/event/surface/window | Completed | Go Verified：provider-independent exact operations、bounded title inspection concurrency 与 literal event scan 由 `session/query/service_test.go` 覆盖 |
| S6-Q04 | Session Query | Session lineage 与 Event relationship trace | Completed | Go Verified：ancestor/descendant、unresolved parent、cycle、replacement chain、source/derived edge 均由同一 logical observation 计算 |
...
| Projection Cache identity/version、suffix/full restore、single-writer、关闭排空与 SQLite replacement | `session/projectioncache`、`session/projectioncache/sqlite` |
| cold `session.list` cache summary、history cut-first reverse page 与 Subagent cached identity seed gate | `apiproxy/session/list_projection_test.go`、`history_cache_test.go`、`subagent/internal/childdirectory/cache_test.go` |
...
| LLM Retry normal/always、budget、Retry-After/backoff、durable history、取消与 drain | `llm/retry/consumer_test.go`、`llm/retry/policy_test.go`、`llm/retry/history_test.go` |
| StreamChunk 增量组装、first-close 和 max-token tool-call 处理 | `llm/assembler_test.go` |
| DeepSeek typed config、环境优先级和 immutable snapshot | `llm/deepseek/config_test.go` |
| DeepSeek message/request serialization 与 image/reasoning/stop 语义 | `llm/deepseek/serialize_test.go` |
| DeepSeek SSE framing、translation、usage、finish、empty/malformed/timeout | `llm/deepseek/stream_test.go` |
...
`go mod tidy` 当前会加载 `//go:build contract` 测试，而这些测试仍导入本 checkout 已不存在的 `tests/contract/fixture`，因此该命令未通过。本轮没有用 Projection Cache 内的替代包掩盖这个独立 contract-suite 缺口。

固定源码 `WebApiClient` 已通过真实 HTTP 完成 Credentials describe/set/unset，并通过真实 HTTP/WebSocket 创建/搜索 Session、选择模型、提交 prompt、驱动 Go Agent Loop、接收 `turn/end`、修改 queue、cancel、rename 和 respond。默认 composition contract 使用 deterministic DeepSeek HTTP oracle，在同一进程验证固定 Client 和内嵌 UI；UI 自动化完成发送、回复、新建/选择 Session、history 恢复、Question 回答和 Agent continuation，并断言 plugin runtime-context 不进入用户消息投影。API Key dialog 已通过 TypeScript 检查和生产构建，但尚未单独执行交互式浏览器行为验收。
