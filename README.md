...
The default server includes a small embedded Web UI for the core conversation flow: configure the DeepSeek API key, create and select sessions, read history, send text, and render streamed Agent replies. It does not port the original React/plugin-based DeepSeek Harness Web product, browser client runtime, DeepSeek Harness SDK, or Python SDK. Headless, ACP, MCP, and Typert-backed auxiliary endpoints are deferred unless a later scope decision includes them.
