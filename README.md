...
The project studies how such an agent can extract configurable knowledge objects from conversation context, retrieve relevant existing knowledge through a read-only interface, and directly return whether the user supports, opposes, or is uncertain about each object. Supported and opposed conclusions distinguish the user's explicit position from the agent's contextual inference, without depending on how knowledge is stored or managed.
