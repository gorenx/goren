...
func TestSerializeMessagesPreservesDeepSeekReplayRules(t *testing.T) {
	...
	user := mustMessage(t, llm.RoleUser, []llm.ContentBlock{
		...
		llm.ToolResultBlock{Type: "tool-result", ToolCallID: "call-1", Content: nil},
	})
...
func TestSerializeMessagesRejectsNestedImage(t *testing.T) {
	...
	entry := mustMessage(t, llm.RoleUser, []llm.ContentBlock{llm.ToolResultBlock{
		Type: "tool-result", ToolCallID: "call-1", Content: []llm.ContentBlock{llm.ImageBlock{
			Type: "image", Attachment: attachment.ImageAttachmentRef{},
		}},
	}})
...
func TestSerializeRequestThinkingAndOptionalFields(t *testing.T) {
	...
	requestOptions := llm.GenerateOptions{
		CallConfig: llm.CallConfig{Model: "m", ReasoningEffort: llm.ReasoningEffortID(ReasoningHigh), MaxTokens: &maximumTokens, Stop: emptyStops},
		Tools:      []llm.ToolSchema{{Name: "lookup", Description: "Lookup", Parameters: json.RawMessage(`{"type":"object"}`)}},
	}
	wireValue, err := SerializeRequest(requestOptions, RequestDefaults{ReasoningEffort: &high})
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = SerializeRequest(requestOptions, RequestDefaults{Thinking: &disabled})
	var providerFailure *llm.LlmError
	...
	wireValue, err = SerializeRequest(requestOptions, RequestDefaults{Thinking: &disabled})
...
func mustMessage(t *testing.T, role llm.MessageRole, content []llm.ContentBlock) llm.Message {
	...
	entry, err := llm.NewMessage(llm.MessageInput{
		Role: role, Content: content, Source: llm.PluginMessageSource{Plugin: "test"},
	})
	if err != nil {
		t.Fatal(err)
	}
